...
for command_name in shasum route ifconfig networksetup ditto sw_vers nohup awk sed; do
...
echo "Environment check passed: macOS version, architecture, root, package, commands, and disk space."
...
rollback() {
  ...
  rm -rf "$BACKUP_DIR"
  echo "Offline macOS client installation failed; the previous local installation was restored." >&2
  exit "$exit_code"
}
trap rollback ERR
...
chmod 700 "$INSTALL_DIR" "$CONFIG_DIR" "$RUN_DIR" "$LOG_DIR"
rm -rf "$RUNTIME_NEW"
mkdir -p "$RUNTIME_NEW"
...
RUNTIME_EXE="$RUNTIME/pyvpn-client/pyvpn-client"
TOOLS_EXE="$RUNTIME/pyvpn-client-tools/pyvpn-client-tools"
...
chown root:wheel "$PROFILES_PATH"
...
DNS_STATE_Q="$(printf '%q' "$DNS_STATE")"
...
DNS_STATE=$DNS_STATE_Q
rm -f "\$STOP_FILE"
ARGS=(--profiles "\$PROFILES_PATH" --stop-file "\$STOP_FILE" --dns-state-file "\$DNS_STATE")
if [[ -n "\${SSH_CLIENT:-}" ]]; then ARGS+=(--bypass-ip "\${SSH_CLIENT%% *}"); fi
...
SERVER_ENDPOINT="\$("\$RUNTIME_EXE" servers --file "\$PROFILES_PATH" show 2>/dev/null | sed -n 's/^server: //p' | head -n 1)"
SERVER_HOST="\${SERVER_ENDPOINT%:*}"
if [[ -n "\$SERVER_HOST" ]]; then
  route -n delete -host "\$SERVER_HOST" >/dev/null 2>&1 || true
  SERVER_IP="\$("\$TOOLS_EXE" resolve "\$SERVER_HOST" 2>/dev/null || true)"
  [[ -z "\$SERVER_IP" ]] || route -n delete -host "\$SERVER_IP" >/dev/null 2>&1 || true
fi
"\$TOOLS_EXE" restore-macos-dns "\$DNS_STATE" || true
...
chown root:wheel "$START_SCRIPT" "$UP_SCRIPT" "$DOWN_SCRIPT" "$STATUS_SCRIPT" "$SERVERS_SCRIPT" "$SWITCH_SCRIPT"
